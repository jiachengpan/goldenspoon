...
class Regress():
    ...
    def data_preprocess_by_date(self):
        total_x_train = pd.DataFrame()
        total_y_train = pd.DataFrame()
        ...
        for date in self.date_list:
            ...
            if not df_indicator.empty:
                ...
                total_x_train = pd.concat([total_x_train, x_train], axis=0)
                total_y_train = pd.concat([total_y_train, y_train], axis=0)

                total_x_test[date] = x_test
                total_y_test[date] = y_test
            else:
                print("{} is a empty dataframe!".format(indicator_pickle))
        return total_x_train, total_x_test, total_y_train, total_y_test

    ## 2. result visualization
    def draw(self, y_pred, y_test, cur_stock_price_test, R2, show_interval=None, prefix=''):
        ...
        while start_ < len(y_pred):
            ...
            flag = f'{prefix}demo_index_{start_}-{end_}.png'
            plt.savefig(os.path.join(self.save_path, flag))
        ...
        plt.savefig(os.path.join(self.save_path, 'y_scatter.png'))
    ...
    def run(self):
        ...
        print(f'running on dates: {self.date_list}')
        print(f'prediction: {self.predict_month_id}')
    ...
    def run_by_date(self):
        ## get train/test data
        x_train, x_test_all, y_train, y_test_all = self.data_preprocess_by_date()
        print(f'running on dates: {self.date_list}')
        print(f'prediction: {self.predict_month_id}')
        print(f'train shape: X: {x_train.shape} Y: {y_train.shape}')

        for k in x_test_all:
            print(f'test shape: k: {k} X: {x_test_all[k].shape} Y: {y_test_all[k].shape}')
        ...
        ## print result
        with open(os.path.join(self.save_path, 'indicator_weight.log'), 'w') as fh:
            fh.write('\n'.join(map(str, indicator_weight)))
        ...
        for k in x_test_all:
            ...
            print(f'{k}: TP: {TP} FP: {FP} TN: {TN} FN: {FN}')
            self.draw(
                    y_pred, y_test,
                    cur_stock_price_test, R2_y_pred_y_test,
                    show_interval=50, prefix=f't{k}_')

        with open(os.path.join(self.save_path, 'summary'), 'w') as fh:
...
if __name__ == '__main__':
    ...
    for predict_month_id in [1,2,3]:
        ...
        for dates in range(len(date_list)):
            ...
            save_path = os.path.join(saveflag, end_date, f'{predict_month_id}month') + '/'
            os.makedirs(save_path, exist_ok=True)

            date_list_chosen = date_list[:dates+1]
            r = Regress(linear_model.LinearRegression(),
                pickle_path,
                save_path,
                date_list_chosen,
                predict_n_month,
