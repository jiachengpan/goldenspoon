...
class Indicator:
    def __init__(self, db, start_date=None, end_date=None):
        self.db = db
        self.start_date = start_date if start_date is None else parser.parse(
            start_date).date()
        self.end_date = end_date if end_date is None else parser.parse(
            end_date).date()
    ...
    def get_stocks_timed(self):
        ...
        columns = [
            c for c in result.columns if c not in self.k_stock_exclude_timed_columns]
        filtered = result[columns]

        if self.start_date:
            filtered = filtered[filtered[self.k_column_date]
                                >= self.start_date]
        if self.end_date:
            filtered = filtered[filtered[self.k_column_date] <= self.end_date]

        return pd.concat([filtered,
                          result[[self.k_column_date, self.k_stock_column_code] +
                                 self.k_stock_exclude_timed_columns],
    ...
    def add_stock_name(self, df):
        if self.k_stock_column_code in df:
            df[self.k_stock_column_name] = df[self.k_stock_column_code].map(
                self.get_stocks_map())
        elif self.k_stock_column_name in df:
            df[self.k_stock_column_code] = df[self.k_stock_column_name].map(
                self.get_stocks_name_map())
        else:
            assert 0, 'invalid df columns: {}'.format(df.columns.tolist())

        columns = [self.k_column_date,
                   self.k_stock_column_code, self.k_stock_column_name]
    ...
    def get_stock_general(self):
        ...
        untimed_columns += [self.k_stock_column_industry.format(level=i) for i in [
            1, 2, 3, 4]]
        ...
        stocks_timed = self.get_stocks_timed()[timed_columns].dropna() \
            ...
            .last() \
            .drop(columns=[self.k_column_date])
    ...
    def get_funds_timed(self):
        ...
        columns = [
            c for c in result.columns if c not in self.k_fund_exclude_timed_columns]
        filtered = result[columns]
        ...
        return pd.concat([filtered,
                          result[[self.k_column_date, self.k_fund_column_code] +
                                 self.k_fund_exclude_timed_columns],
                          ], ignore_index=True, sort=False)
    ...
    def get_funds_manager_stats(self, only_current=True):
        ...
        stats['基金名称'] = stats[self.k_fund_column_code].apply(
            lambda v: set([self.db.get_fund_by_code(vv) for vv in v]))
    ...
    def get_funds_topn_stocks(self):
        fund_topn_stocks = self.db.get_fund_stats('topn_stocks')
        if self.start_date:
            fund_topn_stocks = fund_topn_stocks[fund_topn_stocks[self.k_column_date]
                                                >= self.start_date]
        if self.end_date:
            fund_topn_stocks = fund_topn_stocks[fund_topn_stocks[self.k_column_date]
                                                <= self.end_date]
        ...
        key_columns = [self.k_column_date,
                       ...
                       'indicator']
        fund_topn_stocks = fund_topn_stocks.groupby(
            key_columns).sum().reset_index()
