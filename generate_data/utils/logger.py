...
# Modifications Copyright (c) 2021 Enflame. All rights reserved.
#
...
class Logger(object):
    ...
    def __init__(self, severity=INFO, colors=True, letter=True, timestamp=False, line_info=False):
        ...
        Args:
            ...
            line_info (bool):
                    ...
                    Defaults to False.
            log_file (str):
                    Path to a log file to write logging output.
                    This will not include logging messages from other libraries, like
    ...
    def log(self, message, severity, mode=LogMode.EACH, stack_depth=2, error_ok=False):
        ...
        """
        # from topsideas import constants, config
            ...
            def get_prefix():
                def get_line_info():
                    ...
                    module = inspect.getmodule(
                        sys._getframe(adjusted_stack_depth))
                    ...
                    # If the file is not located in this package, use its basename instead.
        ...
        def should_log(message):
            ...
            # print("!!!!!!!!!! self._severity:",self._severity)
            # self._severity =50
        ...
        if callable(message):
            try:
                ...
                if not error_ok or INTERNAL_CORRECTNESS_CHECKS:
                    raise
                message = "<Error while logging this message: {:}>".format(
                    str(err))
        ...
        if INTERNAL_CORRECTNESS_CHECKS and severity == Logger.WARNING:
            import warnings
        ...
        # print("Logger.CRITICAL:",Logger.CRITICAL)
        
        # print(message, file=sys.stdout if severity <
        #       Logger.VERBOSE else sys.stderr)
        # # assert(0)
        print(message, file=sys.stdout if severity <
              Logger.CRITICAL else sys.stderr)

    def backtrace(self, depth=0, limit=None, severity=ERROR):
        limit = limit if limit is not None else (
            3 - self.severity // 10) * 2  # Info provides 1 stack frame
        limit = max(limit, 0)
        self.log(" ".join(traceback.format_stack(
            f=sys._getframe(depth + 2), limit=limit)), severity=severity)

    def verbose(self, message, mode=LogMode.EACH):
        self.log(message, Logger.VERBOSE, mode=mode,
                 stack_depth=3, error_ok=True)
    ...
    def critical(self, message):
        ...
        raise KnownException(message) from None

    def internal_error(self, message):
        if INTERNAL_CORRECTNESS_CHECKS:
            self.log(message, Logger.CRITICAL, stack_depth=3)

            raise InternalException(message) from None
    ...
    def _str_from_module_info(self, module, name=None):
        ...
        try_append(
            lambda: name or "Loaded Module: {:<18}".format(module.__name__))
        try_append(lambda: " | Version: {:<8}".format(module.__version__))
        try_append(lambda: " | Path: {:}".format(
            list(map(os.path.realpath, module.__path__))))
        return ret

    def module_info(self, module, name=None, severity=VERBOSE):
        self.log(self._str_from_module_info(module, name),
                 severity=severity, mode=LogMode.ONCE)
