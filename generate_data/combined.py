#!/usr/bin/env python
# coding: utf-8
from data_utils import Database, Indicator
import os
import sys
import pandas as pd
import numpy as np
...
k_cyclical_industry_keys = ['原材料', '汽车与汽车零部件', '房地产',
                            '交通运输', '能源', '资本市场', '消费者服务', '银行', '其他金融', '保险']
...
class stocks_dynamic_indicators():
            ...
            for stock_id in industry_stock_id:
                ...
                if ind_stocks_quater_count > 0:
                    ...
                    if ind_stocks_quater_count >= self.past_quater_number:
                        baseline = ind_stocks_perf_temp.iloc[0:self.past_quater_number - 1].mean()[
                            perf_metric]
                        sample = sample[:self.past_quater_number]
                    else:
                        baseline = ind_stocks_perf_temp.iloc[0:ind_stocks_quater_count-1].mean()[
                            perf_metric]
    ...
    def q_quater_fund_owner_affinity_mean_std(self, perf_metric):
        ...
        for industry in firstindustry_count.keys():
            ...
            for stock_id in industry_stock_id:
                ind_stocks_perf_with_fund_temp = (
                    self.ind_stocks_perf_with_fund.loc[self.ind_stocks_perf_with_fund['股票代码'] == stock_id])
                ind_stocks_perf_temp = (
                    self.ind_stocks_perf.loc[self.ind_stocks_perf['股票代码'] == stock_id]).sort_values(by=['日期'])
                ind_stocks_quater_count = ind_stocks_perf_temp.shape[0]
                ...
                for quater in date:
                    quater_sample = ind_stocks_perf_with_fund_temp.loc[
                        ind_stocks_perf_with_fund_temp['日期'] == quater]
                    ...
                    if fund_owner_sample.shape[0] >= self.minimal_fund_number:
                        ...
                        quater_fund_owner_affinity = 1 - \
                            fund_owner_sample_unique.shape[0] / \
                            fund_owner_sample.shape[0]
                        stock_quater_fund_owner_affinity.append(
                            quater_fund_owner_affinity)
                stock_quater_fund_owner_affinity = np.asarray(
                    stock_quater_fund_owner_affinity)
            ...
            for stock_id in industry_stock_id:
                ...
                for quater in date:
                    ...
                    if fund_revisit_sample.shape[0] >= self.minimal_fund_number:
                        ...
                        stock_quater_fund_revist_unique.append(
                            fund_revisit_sample_unique)
                if len(stock_quater_fund_revist) > 0:
                    revisit_ratio = (
                        1-len(stock_quater_fund_revist_unique)/len(stock_quater_fund_revist))
                stock_id_list.append(stock_id)
    ...
    def stock_cyclical_industry(self, cyclical_industry_keys):
        ...
        secondindustry_count = compute_secondindustry_count(
            self.ind_stocks_general)

        for industry in secondindustry_count.keys():
            industry_stocks_general = self.ind_stocks_general.loc[
                self.ind_stocks_general['所属中证行业(2016) [行业类别]2级'] == industry]
            industry_stock_id = industry_stocks_general['股票代码']
            cyclical_insdustry = []
            for stock_id in industry_stock_id:
...
class stocks_industry_static_indicators():
    ...
    def stocks_industry_static_indicators(self, industry_metric='股票风格分类'):
        indicator_types = self.ind_stocks_general_new[industry_metric].drop_duplicates(
        ).values.tolist()
        dict_industry_indicator = {}

        firstindustry_count = compute_firstindustry_count(
            self.ind_stocks_general_new)

        for firstindustry_key in firstindustry_count.keys():
            dict_indicator_percent = {}
            for type in indicator_types:
                firstindustry_indicator_count = self.ind_stocks_general_new[(
                    self.ind_stocks_general_new[self.industry_level] == firstindustry_key) & (self.ind_stocks_general_new[industry_metric] == type)]
                firstindustry_indicator_percent = firstindustry_indicator_count['股票代码'].count(
                ) / firstindustry_count[firstindustry_key]
                dict_indicator_percent[type] = float(
                    firstindustry_indicator_percent)
        ...
        def function(stock_id, stock_industry):
            ...
            return
        self.ind_stocks_general_new.apply(lambda x: function(
            x['股票代码'], x[self.industry_level]), axis=1)  # ind_stocks_general_new存在
...
def get_stocks_industry_static_indicators(ind_stocks_general_new):
    ...
    stocks_industry_static = stocks_industry_static_indicators(
        ind_stocks_general_new, industry_level)

    indicator_stock_hushen300 = stocks_industry_static.stocks_industry_static_indicators(
        industry_metric='是否属于重要指数成份 [所属指数]沪深300指数')
    indicator_stock_styles = stocks_industry_static.stocks_industry_static_indicators(
        industry_metric='股票风格分类')
    indicator_stock_scales = stocks_industry_static.stocks_industry_static_indicators(
        industry_metric='股票规模指标')
...
def get_stocks_industry_dynamic_indicators(ind_stocks_general, ind_stocks_perf, ind_stocks_perf_holding_funds_share):
    ...
    stocks_industry_dynamic = stocks_industry_dynamic_indicators(
        ind_stocks_general, past_quater_number)
    ...
    avg_price_stock_id_list, avg_price_mean, avg_price_std = stocks_industry_dynamic.q_quater_mean_std(
        '当月成交均价 [复权方式]不复权', ind_stocks_perf)
    ...
    df_avg_price = meanstd_list_to_df(
        avg_price_stock_id_list, avg_price_mean, avg_price_std, flag='avg_price')
...
def get_stocks_dynamic_indicators(ind_stocks_general, ind_stocks_perf, ind_stocks_holding_funds_share, ind_stocks_perf_with_fund):
    ...
    stocks_dynamic = stocks_dynamic_indicators(
        ind_stocks_general, ind_stocks_perf, ind_stocks_perf_with_fund, past_quater_number, minimal_fund_number)

    close_price_stock_id_list, close_price_mean, close_price_std = stocks_dynamic.q_quater_mean_std(
        '月收盘价 [复权方式]不复权', ind_stocks_perf)  # data base:ind_stocks_perf
    avg_price_stock_id_list, avg_price_mean, avg_price_std = stocks_dynamic.q_quater_mean_std(
        '当月成交均价 [复权方式]不复权', ind_stocks_perf)  # data base:ind_stocks_perf
    turnover_rate_stock_id_list, turnover_rate_mean, turnover_rate_std = stocks_dynamic.q_quater_mean_std(
        '月换手率 [单位]%', ind_stocks_perf)  # data base:ind_stocks_perf
    ...
    df_close_price = meanstd_list_to_df(
        close_price_stock_id_list, close_price_mean, close_price_std, flag='close_price')
    df_avg_price = meanstd_list_to_df(
        avg_price_stock_id_list, avg_price_mean, avg_price_std, flag='avg_price')
    df_turnover_rate = meanstd_list_to_df(
        turnover_rate_stock_id_list, turnover_rate_mean, turnover_rate_std, flag='turnover_rate')
    df_amplitutde = meanstd_list_to_df(
    ...
    df_fund_revisit = cumstomlist_to_df(
        fund_revisit_stock_id_list, fund_revisit_mean, flags=['fund_revisit'])
    df_cyclical_industry = cumstomlist_to_df(
    ...
    dfs = [df_close_price, df_avg_price, df_turnover_rate, df_amplitutde, df_margin_diff,
           df_share_ratio_of_funds, df_num_of_funds, df_fund_owner_affinity, df_fund_revisit, df_cyclical_industry]
...
def compute_indicators(db):
    ...
    stocks_scale = db.get_stock_stats('date')[['股票代码', '股票规模指标']].dropna().groupby([
        '股票代码']).last().reset_index()
    stocks_scale.columns = ['股票代码', '股票规模指标']
    stocks_hushen300 = db.get_stocks()[['股票代码', '是否属于重要指数成份 [所属指数]沪深300指数']]
    ind_stocks_general_new = pd.merge(
        ind_stocks_general, stocks_scale, how='inner', on=['股票代码'])
    ind_stocks_general_new = pd.merge(
        ind_stocks_general_new, stocks_hushen300, how='inner', on=['股票代码'])
    ind_stocks_general_new['是否属于重要指数成份 [所属指数]沪深300指数'].fillna(
        '否', inplace=True)

    # for class stocks_industry_dynamic_indicators xy
    ind_stocks_perf_holding_funds_share = pd.merge(
        ind_stocks_perf, ind_stocks_holding_funds_share, how='outer', on=['日期', '股票代码'])
    ind_stocks_perf_holding_funds_share['基金持股总值'] = ind_stocks_perf_holding_funds_share['基金持股数量 [单位]股'] * \
        ind_stocks_perf_holding_funds_share['月收盘价 [复权方式]不复权']
    ...
    print("df_stocks_industry_static_indicators:",
    ...
    print("df_stocks_industry_dynamic_indicators:",
