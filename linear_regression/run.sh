...
# train_date_len=3
# train_date_list="2020-09-30 2020-12-31 2021-03-31"
...
################################################################################
data_flag=label_zero-new # [label_zero-pre,label_zero-cur,label_one_month_prior]
data_path=regress_data-${data_flag}/past_quarters_4/
result_flag=regress_result-${test_date_list}

label_type=class # ['regress','class']
training_model=gbdtclassifier # gbdtclassifier #adaboostclassifier # votingclassifier     # ['linear', 'ridge', 'lasso', 'randomforest', 'randomforestclssifier', 'adaboostregressor', 'adaboostclassifier']
indicator_use_type=all # ['all','all_with_premonth_label', 'industrial_static', 'industrial_dynamic', 'stock_dynamic', 'static_stock_dynamic','all_dynamic']
...
if [ "${run_with_standardscaler}" = "T" ]; then
  savepath=${savepath}/run_with_standardscaler/
  data_standardscaler='--data_standardscaler'
else
  savepath=${savepath}/run/
  data_standardscaler=''
fi
