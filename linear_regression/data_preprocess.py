...
class DataPreprocess():
        ...
        for m in range(len(df_label_list)):
            ...
            if m==0:
                ...
                if '_predict_changerate_price' in self.i_month_label:
                    df_label.columns = ['id', 'date', "predict_absvalue_price", str(m)+'_predict_changerate_price']
                    df_label = df_label.drop(columns=['date'], axis=1)
                    df_label = df_label.drop(columns=['predict_absvalue_price'], axis=1)
                elif '_predict_absvalue_price' in self.i_month_label:
                    df_label.columns = ['id', 'date', str(m)+'_predict_absvalue_price', "predict_changerate_price"]
                    df_label = df_label.drop(columns=['date'], axis=1)
                    df_label = df_label.drop(columns=['predict_changerate_price'], axis=1)
    ...
    def get_train_indicator_label(self, df_indicator_label):
        ...
        Y_df = df_indicator_label.iloc[:,df_indicator_label.columns.str.endswith(self.i_month_label)]
        ...
        for date in self.train_date_list:
            ...
            if not df_indicator.empty:
                ...
                for m in range(self.n_month_predict+1): ## include 0_month
                    label_pickle = self.data_path + 'labels.' + str(m) + '_month.'+ date +'.pickle'
