...
def get_args():
    ...
    parser.add_argument("--predict_mode", default="predict_changerate_price", choices=['predict_changerate_price','predict_absvalue_price'])
...
class Regress():
    ...
    def run(self,data_path,train_date_list,test_date_list,n_month_predict):
        ## 数据预处理
        x_train, x_test, y_train, y_test, test_ID_df = DataPreprocess(data_path, train_date_list,test_date_list,n_month_predict,self.i_month_label,self.indicator_list).run()
        ...
        indicator_weight = list(zip(self.indicator_list, list(self.model.coef_)))
        ...
        ## 结果分析
        valid_stock_list, y_pred_valid, y_true_valid, y_stock_id_valid = drop_small_change_stock(y_pred=y_pred, y_true=y_test, drop_ponit=0.15, y_stock_id=y_testID)
        full_stock_list = y_test.shape[0]
        # TP, FP, TN, FN = perf_measure(y_pred, y_test, cur_stock_price_test)
        TP, FP, TN, FN = perf_measure(y_pred_valid, y_true_valid, cur_stock_price_test, self.i_month_label, self.save_path_permonth)
        stock_pred_correctness = perf_measure_per_stock(full_stock_list, valid_stock_list, y_pred_valid, y_true_valid, y_stock_id_valid)
...
if __name__ == '__main__':
    ...
    predict_mode = args.predict_mode #'predict_changerate_price' or 'predict_absvalue_price'
    regress_model = linear_model.LinearRegression()

    # 如果没有指定使用的indicator列表，则使用全部的indicator训练
    if indicator_use_list == []:
        indicator_pickle = args.data_path + 'indicators.' + train_date_list[0] + '.pickle'
        df_indicator = pd.read_pickle(indicator_pickle)
        all_indicator_list = list(df_indicator.columns.values)[1:] # exclude 'id'
    ...
    for i in range(n_month_predict):
        ...
        r = Regress(regress_model, save_path_permonth, i_month_label, indicator_list)
